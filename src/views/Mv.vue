<template>
  <div class="mvDetail">
    <!-- 头部  -->
    <div class="title">
      <span class="one" @click="going">‹</span><span class="two">Mv</span>
    </div>
    <div class="mvideo">
      <video
        :src="mvurl"
...
export default {
  ...
  created() {
    ...
    let likeList = JSON.parse(window.localStorage.getItem("likeMvList"));
    if (likeList) {
  ...
  async mounted() {
    // this.$http
    //   .get("http://106.52.43.165:3001/mv/url?id=" + this.mvId)
    //   .then(data => {
    ...
    //     console.log(err);
    //   });
    let dataone = await axios.get(
      "http://106.52.43.165:3001/mv/url?id=" + this.mvId
    );
    this.mvurl = dataone.data.url;
    //  console.log(dataone);
    // this.$http
    //   .get("http://106.52.43.165:3001/comment/mv?id=" + this.mvId)
    //   .then(data => {
    ...
    let datatwo = await axios.get(
      "http://106.52.43.165:3001/comment/mv?id=" + this.mvId
    );
    console.log(datatwo);
    this.isLoading = false;
    this.commentList = datatwo.comments;
    this.hotComments = datatwo.hotComments;
  ...
  methods: {
    ...
    like() {
      ...
      let newLike = JSON.parse(window.localStorage.getItem("likeMvList"));
      if (newLike) {
        if (this.likeState) {
          newLike.push(this.mvId);
          newLike = Array.from(new Set(newLike));
        } else {
          let index = newLike.indexOf(this.mvId);
      ...
      window.localStorage.setItem("likeMvList", JSON.stringify(newLike));
    }
  }
};
</script>
